...
def go_somewhere(_, _1):
    clear()
    os.system("python " + _1 + "/" + os.listdir(_1)[_ - 2])
    input("\npress enter to go to main menu\n")


def output(list1):
    for f in range(len(os.listdir(list1))):
        print(f + 1, "--", os.listdir(list1)[f - 1])
    x = int(
        input(
            "input the number of the project you want to run\nnote all programms wth 'extra' in their names is just that... they are not to be used for marking\n"
        ))
    return x
