...
def paths(x):
    ...
    return sorted([(i) for i in list(os.listdir(x))])


def add():
    v = input(
        "please input the item and then the amount you want to add separated by a space and beginning with an x\nif multiple items need to be added please follow that layout by putting a space between each item ie 'peanuts x10 apples x2 pears bananas'\n"
    )
    v = v.split()
...
def delete():
    v = input(
        "please input the item and then the amount you want to delete separated by a space and beginning with an x\nif multiple items need to be added please follow that layout by putting a space between each item ie 'peanuts x10 apples x2 pears bananas'\n"
    )
    v = v.split()
    for x in range(len(v)):
        try:
