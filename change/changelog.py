...
def a(vm):
    if vm==True:return
    un = [".upm", "extra", ".git", "change"];f = open("change/change.json", "r");done = json.load(f);lists = sorted([(i)for i in list(filter(os.path.isdir, os.listdir("/home/runner/obs-all")))if i not in un]);pop=1
    if pop==1:
        for x in lists:
