import os, json,time,random;clear = lambda: os.system("clear");un = [".upm", "extra", ".git", "change"];f = open("change/change.json", "r");done = json.load(f);lists = sorted([(i)for i in list(filter(os.path.isdir, os.listdir("/home/runner/obs-all")))if i not in un]);animation = "|/-\\";pop=0
if pop==1:
    for x in lists:
        for v in os.listdir(x):
            print("checking");m=str(x+" "+v);print(m);time.sleep(random.uniform(0.05,0.2));print("passed");time.sleep(0.1);clear()
